...
import { useWebsiteTheme, ColorMode } from "@/components/providers/WebsiteThemeProvider";
import useMounted from "@/hooks/useMounted";
import { getSettingsMenuSections, SettingsItemType, SettingsScope } from "@/config/settings-menu";
...
function ThemeOptionButton({
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 text-left group-hover:text-foreground transition-colors">{label}</span>
...
function ThemeSubmenu({
  ...
  const { websiteTheme, setWebsiteTheme, colorMode, setColorMode } = useWebsiteTheme();
      ...
      <>
        {(["light", "dark", "system"] as const).map((mode) => (
          <ThemeOptionButton
            ...
            color={mode === "light" ? "#ffffff" : mode === "dark" ? "#000000" : "#6b7280"}
    ...
    <>
      {CODE_THEMES.map((theme) => (
        <ThemeOptionButton
          ...
          label={theme.label}
          color={theme.color}
          icon={EDITOR_THEME_ICONS[theme.id] ?? <CircleOff className="size-4" />}
...
function SettingsMenu({ scope, triggerClassName }: SettingsMenuProps) {
  ...
  const [activeSubmenu, setActiveSubmenu] = useState<SettingsItemType | null>(null);
                ...
                {section.items.map((item) => (
                  ...
                  >
                    <button
                      ...
                      `}
                      onClick={() =>
                        setActiveSubmenu((current) => (current === item.type ? null : item.type))
                      }
                    >
                    ...
                    <AnimatePresence>
                      {activeSubmenu === item.type && (
                        <>
                          ...
                          <motion.div
                            initial={{ opacity: 0, x: submenuSide === "right" ? 4 : -4, scale: 0.98 }}
                            animate={{ opacity: 1, x: 0, scale: 1 }}
                            exit={{ opacity: 0, x: submenuSide === "right" ? 4 : -4, scale: 0.98 }}
